...
class RandomCameraIterableDataset(IterableDataset, Updateable):
    ...
    def __init__(self, cfg: Any) -> None:
        ...
        self.batch_sizes: List[int] = ([self.cfg.batch_size]
                                       if isinstance(self.cfg.batch_size, int) else self.cfg.batch_size)
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        if random.random() < 0.5:
            ...
            elevation_deg = (torch.rand(self.batch_size) * (self.elevation_range[1] - self.elevation_range[0]) +
            ...
            elevation = torch.asin((torch.rand(self.batch_size) *
        ...
        if self.cfg.batch_uniform_azimuth:
            ...
            azimuth_deg = (torch.rand(self.batch_size) * (self.azimuth_range[1] - self.azimuth_range[0]) +
                           self.azimuth_range[0])
        azimuth = azimuth_deg * math.pi / 180

        # sample distances from a uniform distribution bounded by distance_range
        camera_distances: Float[Tensor, "B"] = (torch.rand(self.batch_size) *
        ...
        camera_perturb: Float[Tensor, "B 3"] = (torch.rand(self.batch_size, 3) * 2 * self.cfg.camera_perturb -
        ...
        fovy_deg: Float[Tensor, "B"] = (torch.rand(self.batch_size) * (self.fovy_range[1] - self.fovy_range[0]) +
                                        self.fovy_range[0])
        fovy = fovy_deg * math.pi / 180

        # sample light distance from a uniform distribution bounded by light_distance_range
        light_distances: Float[Tensor, "B"] = (torch.rand(self.batch_size) *
        ...
        directions: Float[Tensor,
                          "B H W 3"] = self.directions_unit_focal[None, :, :, :].repeat(self.batch_size, 1, 1, 1)
        ...
        self.proj_mtx: Float[Tensor, "B 4 4"] = get_projection_matrix(fovy, self.width / self.height, 0.01,
                                                                      100.0)  # FIXME: hard-coded near and far
...
class RandomCameraDataset(Dataset):
    ...
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        center: Float[Tensor, "B 3"] = torch.zeros_like(camera_positions)
        # default camera up direction as +z
        up: Float[Tensor, "B 3"] = torch.as_tensor([0, 0, 1],
                                                   dtype=torch.float32)[None, :].repeat(self.cfg.eval_batch_size, 1)
        ...
        rays_o, rays_d = get_rays(directions, c2w, keepdim=True, normalize=self.cfg.rays_d_normalize)
        self.proj_mtx: Float[Tensor, "B 4 4"] = get_projection_matrix(fovy, self.cfg.eval_width / self.cfg.eval_height,
                                                                      0.01, 100.0)  # FIXME: hard-coded near and far
        mvp_mtx: Float[Tensor, "B 4 4"] = get_mvp_matrix(c2w, self.proj_mtx)
